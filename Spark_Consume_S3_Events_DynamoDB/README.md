...
- DynamoDB is a good option here being serverless with on-demand capacity features and can also help in reporting on events but SQS, Kinesis DataStreams, Kafka etc. could also be effectively used here.
...
- As Spark is only processing events from DynamoDB, this process has Automatic Bookmarking on what files are already processed and each new run only processes PENDING events. Adding Step Concurrency in EMR to 1 can ensure that only one instance of this job runs at any one time. 
